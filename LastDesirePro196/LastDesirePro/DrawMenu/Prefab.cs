  ...
  public static class Prefab {
    ...
    static Prefab() {
      ...
      style = new GUIStyle(GUI.skin.label) {
        fontSize = 12, font = LastDesirePro.Menu.MainMenu._TextFont
      };
      outlineStyle = new GUIStyle(GUI.skin.label) {
        fontSize = 12, font = LastDesirePro.Menu.MainMenu._TextFont
      };
      _TextMainMenu = new GUIStyle(GUI.skin.label) {
        font = Menu.MainMenu._TabFont, fontSize = 30
      };
    ...
    public static void String(Vector2 pos, string text, Color color, bool center = true, int size = 12, FontStyle fontStyle = FontStyle.Bold, int depth = 1) {
      ...
      outlineStyle.normal.textColor = new Color(0 f, 0 f, 0 f, 1 f);
      outlineStyle.fontStyle = fontStyle;
      GUIContent content = new GUIContent(text);
      GUIContent content2 = new GUIContent(text);
      if (center)
        pos.x -= style.CalcSize(content).x / 2 f;
      switch (depth) {
      case 0:

        GUI.Label(new Rect(pos.x, pos.y, 300 f, 25 f), content, style);
        break;
      case 1:
        GUI.Label(new Rect(pos.x + 1 f, pos.y + 1 f, 300 f, 25 f), content2, outlineStyle);
        GUI.Label(new Rect(pos.x, pos.y, 300 f, 25 f), content, style);
        break;
      case 2:
        GUI.Label(new Rect(pos.x, pos.y, 300 f, 25 f), content, leftstyle);
        break;
      }
    }
    ...
    public static KeyCode Bind(KeyCode key, string text, ref bool bind) {
      ...
      using(new GUILayout.HorizontalScope()) {
        ...
        GUILayout.EndVertical();
        KeyCode lastkey;
        if (Button(!bind ? ((key == KeyCode.None) ? "<b>...</b>" : Convertor.KeyNames[key]) : "Set key", 70 f, 25 f))
          bind = true;
        if (bind) {
    ...
    public static int Tab(float x, float y, float w, float h, string[] text, int list, float WH = 137) {
      for (int i = 0; i < text.Length; i++) {
        ...
        if (list == i)
          Drawing.DrawRect(new Rect(x + (i * WH), y + 20, w, 2), new Color(0.67 f, 0.45 f, 0.9 f, 1 f), 0);
      }
      return list;
    }
    public static int MainTab(float x, float y, float w, float h, string[] text, int list, float WH = 137) {
      for (int i = 0; i < text.Length; i++) {
        ...
        Drawing.DrawRect(new Rect(x + (i * WH), y + 30, w, 2), list == i ? new Color(0.67 f, 0.45 f, 0.9 f, 1 f) : new Color(1, 1, 1, 0.5 f), 5 f);
    ...
    public static string TextField(string text, string label, int width) {
      GUILayout.BeginHorizontal(); {
        ...
        Drawing.DrawRect(new Rect(Loutline.x, Loutline.y + 4, Loutline.width, Loutline.height + 1), new Color(0.67 f, 0.45 f, 0.9 f, 1 f), 2);
