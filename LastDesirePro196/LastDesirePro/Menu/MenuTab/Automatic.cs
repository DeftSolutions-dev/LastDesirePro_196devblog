    ...
    public static void DoAutomatic() {
      ...
      case 0: {
        ScrollViewMenu(new Rect(15, 85, 750, 415), ref _AutoFarm, () => {
          ...
          if (_farmBot[0]) {
            ...
            Slider(0, 2 f, ref _farmBotSpeed, 200, $ "Speed Bot: {string.Format( " {
                0: 0. #
              }
              m / sec ",(float)_farmBotSpeed+4f)}");
      ...
      case 4: {
        ScrollViewMenu(new Rect(15, 85, 750, 415), ref _Misc, () => {
          ...
          if (_Grade) {
            Slider(0 f, 4 f, ref _buildGradeDist, 200, $ "{string.Format(" {
                0
              }
              m.
              ", (int)_buildGradeDist)}.");
            ...
            using(new GUILayout.HorizontalScope()) {
              GUIContent[] target = {
                new GUIContent("Wood"),
                new GUIContent("Stone"),
                new GUIContent("Metal"),
                new GUIContent("TopTier")
              };
              if (Button("◄", 25)) {
                _numGrade--;
                if (_numGrade == -1)
